...
void init_twin_data(twin_properties_t* twin_properties)
{
    ...
    twin_properties->reported_led_yellow   = LED_TWIN_NO_CHANGE;
...
void check_led_status(twin_properties_t* twin_properties)
{
    ...
    if (led_status.change_flag.as_uint16 != LED_FLAG_EMPTY || force_sync)
    {
        ...
        if (led_status.change_flag.yellow == 1 || force_sync)
        {
            if ((led_status.state_flag.yellow & (LED_STATE_BLINK_SLOW | LED_STATE_BLINK_FAST)) != 0)
            {
                twin_properties_ptr->reported_led_yellow = LED_TWIN_BLINK;
            }
            else if (led_status.state_flag.yellow == LED_STATE_HOLD)
            {
                twin_properties_ptr->reported_led_yellow = LED_TWIN_ON;
            }
            else
            {
                twin_properties_ptr->reported_led_yellow = LED_TWIN_OFF;
            }
        }
    ...
    uint16_t* out_response_status)
{
    ...
    *out_response_status = AZ_IOT_STATUS_SERVER_ERROR;
    ...
    else if (spanSize > SERCOM3_USART_WRITE_BUFFER_SIZE)
    {
    ...
    twin_properties_t* twin_properties)
{
    ...
    // Add Green LED
    if (twin_properties->flag.is_initial_get || twin_properties->reported_led_yellow != LED_TWIN_NO_CHANGE)
    {
        if (az_result_failed(
                rc = append_json_property_int32(
                    &jw,
                    led_green_property_name_span,
                    twin_properties->reported_led_yellow)))
        {
