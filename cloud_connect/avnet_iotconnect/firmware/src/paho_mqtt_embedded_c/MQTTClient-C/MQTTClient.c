...
static int sendPacket(MQTTClient* c, int length, Timer* timer)
{
    ...
    if (sent == length)
    {
        TimerCountdown(&c->ping_timer, c->keepAliveInterval); // record the fact that we have MQTT_SUCCESSfully sent the packet
        rc = MQTT_SUCCESS;
    }
    else
        rc = MQTT_FAILURE;
...
int MQTTYield(MQTTClient* c, int timeout_ms)
{
	...
	do
    {
        if (cycle(c, &timer) == MQTT_FAILURE || c->ipstack->evt == MQTT_DISCONNECTED_EVENT)
        {
            rc = MQTT_FAILURE;
            break;
        }
